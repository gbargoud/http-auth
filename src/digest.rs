...
impl Qop {
    ...
    fn h_a2(&self, algorithm: &Algorithm, p: &PasswordParams) -> String {
        match self {
            Qop::Auth => algorithm.h(&[p.method.as_bytes(), b":", p.uri.as_bytes()]),
            Qop::AuthInt => algorithm.h(&[
                p.method.as_bytes(),
                b":",
                p.uri.as_bytes(),
                b":",
...
impl DigestClient {
    ...
    fn respond_inner(&mut self, p: &PasswordParams, cnonce: &str) -> Result<String, String> {
        ...
        let hashed_credentials = self
            .algorithm
            .digest_credentials(p.username, realm, p.password);
        // Select the best available qop and calculate H(A2) as in
        // [https://datatracker.ietf.org/doc/html/rfc7616#section-3.4.3].
        let qop = if let (Some(_body), true) = (p.body, self.qop & Qop::AuthInt) {
            Qop::AuthInt
        ...
        let h_a2 = qop.h_a2(&self.algorithm, p);
...
struct Rfc2617DigestParams<'a> {
    ...
    cnonce: &'a str,
    /// H(A2) as defined in [https://datatracker.ietf.org/doc/html/rfc7616#section-3.4.3]
    h_a2: &'a str,
    /// The QoP to be used for this digest
    qop: &'a Qop,
...
impl<R> DigestServer<R> {
    ...
    pub fn parse_response(&self, r: R, response: &str, password_params: &PasswordParams) -> Result<DigestCredentials, AuthError> {
        ...
        let parsed = ChallengeParser::new(response)
            .next()
            .ok_or(AuthError::MalformedRequest)?
            .map_err(|_err| AuthError::MalformedRequest)?;
        ...
        for (k, v) in &parsed.params {
            if store_param(k, v, "username", &mut username, &mut buf_len)
                ...
                || store_param(k, v, "realm", &mut realm, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
                || store_param(k, v, "uri", &mut uri, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
                || store_param(k, v, "nonce", &mut nonce, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
                || store_param(k, v, "cnonce", &mut cnonce, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
                || store_param(k, v, "response", &mut response, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
                || store_param(k, v, "opaque", &mut opaque, &mut buf_len)
                    .map_err(|_| AuthError::MalformedRequest)?
            {
